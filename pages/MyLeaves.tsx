      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm relative overflow-hidden flex flex-col justify-between">
          ...
          <div className="relative z-10">
            ...
            <div className="flex items-end mt-2">
              <span className="text-3xl font-bold text-gray-900">{annualLeaveLimit - annualUsed}</span>
              <span className="text-sm text-gray-400 mb-1 ml-1">/ {annualLeaveLimit} {t('my.unit.daysLeft')}</span>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-1.5 mt-3">
              <div className="bg-blue-500 h-1.5 rounded-full" style={{ width: `${(annualUsed / annualLeaveLimit) * 100}%` }}></div>
            </div>
          </div>
        ...
        <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm flex flex-col justify-between">
          <div>
            ...
            <div className="flex items-end mt-2">
              <span className="text-3xl font-bold text-gray-900">{publicHolidayCount - publicUsed}</span>
              <span className="text-sm text-gray-400 mb-1 ml-1">/ {publicHolidayCount} {t('my.unit.daysLeft')}</span>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-1.5 mt-3">
              <div className="bg-green-500 h-1.5 rounded-full" style={{ width: `${(publicUsed / publicHolidayCount) * 100}%` }}></div>
            </div>
          </div>
          <div className="mt-4 pt-4 border-t border-gray-50 flex justify-between text-xs text-gray-500">
                ...
                sortedRequests.map((req) => (
                  <tr key={req.id} className="bg-white border-b hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium border ${req.status === 'Approved' ? 'bg-green-100 text-green-800 border-green-200' :
